...
foreach($crafting_meta as $id=>$crafting)
{
	...
	{
		$link=$base_datapack_site_http.$base_datapack_explorer_site_path.'items/'.text_operation_do_for_url($item_meta[$crafting['itemToLearn']]['name'][$current_lang]).'.html';
        ...
        foreach($crafting['material'] as $material=>$quantity)
        {
            ...
            if($item_meta[$material]['image']!='' && file_exists($datapack_path.'items/'.$item_meta[$material]['image']))
                $map_descriptor.='[[Items:'.$item_meta[$material]['name'][$current_lang].'|<img src="'.$base_datapack_site_http.$base_datapack_site_path.'items/'.$item_meta[$material]['image'].'" width="24" height="24" alt="'.$item_meta[$material]['name'][$current_lang].'" title="'.$item_meta[$material]['name'][$current_lang].'" />]]'."\n";
            $map_descriptor.='</td><td>[[Items:'.$item_meta[$material]['name'][$current_lang].'|'."\n";
            if($quantity>1)
        ...
        if($item_meta[$crafting['doItemId']]['image']!='' && file_exists($datapack_path.'items/'.$item_meta[$crafting['doItemId']]['image']))
            $map_descriptor.='[[Items:'.$item_meta[$crafting['doItemId']]['name'][$current_lang].'|<img src="'.$base_datapack_site_http.$base_datapack_site_path.'items/'.$item_meta[$crafting['doItemId']]['image'].'" width="24" height="24" alt="'.$item_meta[$crafting['doItemId']]['name'][$current_lang].'" title="'.$item_meta[$crafting['doItemId']]['name'][$current_lang].'" />]]'."\n";
        $map_descriptor.='</td><td>[[Items:'.$item_meta[$crafting['doItemId']]['name'][$current_lang].'|'.$item_meta[$crafting['doItemId']]['name'][$current_lang].']]</td></tr></table>'."\n";
        $map_descriptor.='</td>'."\n";
